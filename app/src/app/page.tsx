...
export default function HomePage() {
  ...
  const executiveCockpit = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard title="Total Production" value="RM 2.4B" status="neutral" />
        <KPICard title="Safety Incidents" value="7" status="warning" />
        <KPICard title="Anomalies Detected" value="3" status="danger" />
        <KPICard title="Active Platforms" value="124" status="neutral" />
      </div>
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <Chart
          ...
          title="Safety Incidents by Platform"
        />
      </div>
      <DataTable
        columns={[
  ...
  const domainTab1 = (
    <div className="space-y-6">
      ...
      <Chart
        ...
        title="Gas Esg Performance Trend"
        height={400}
      />
    </div>
  );
  ...
  const domainTab2 = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        />
        <ActionMemo
          persona={{ name: 'Malaysia Operations Lead', role: 'Director of Gas Esg' }}
          context={{}}
          onGenerate={async () => ({
            subject: 'Action Required',
            body: 'AI-generated recommendation based on current data patterns and predicted trends.',
            urgency: 'HIGH',
            actions: ['Review top safety incidents findings', 'Optimize platform allocation', 'Prepare quarterly gas esg report'],
          })}
        />
      </div>
    </div>
  ...
  const askAiTab = (
    <div className="h-[600px]">
      <AskAI
        title="Ask AI"
        sampleQuestions={[
          'Which platforms have the highest safety incidents?',
  ...
  const tabs = [
    ...
    { id: 'domain-1', label: 'Gas Esg Analytics', icon: '📈', content: domainTab1 },
